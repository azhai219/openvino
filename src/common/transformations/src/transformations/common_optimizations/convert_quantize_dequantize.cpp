...
ov::pass::ConvertQuantizeDequantize::ConvertQuantizeDequantize() {
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        auto output_low =
            std::dynamic_pointer_cast<opset4::Constant>(pattern_map[output_low_pattern].get_node_shared_ptr());
        if (!output_low)
            return false;
        auto output_high =
            std::dynamic_pointer_cast<opset4::Constant>(pattern_map[output_high_pattern].get_node_shared_ptr());
        if (!output_high)
            return false;
